...
export class Logger {
  ...
  private logFileHandle?: fs.promises.FileHandle;
  ...
  async initialize(config?: LoggerConfig): Promise<void> {
    ...
    if (process.env.LOG_ROTATION_STRATEGY) {
      this.config.rotationStrategy = process.env.LOG_ROTATION_STRATEGY as
        | "truncate"
        | "rotate";
    }
    ...
    if (process.env.LOG_FORMAT) {
      this.config.format = process.env.LOG_FORMAT as "text" | "json";
    }
  ...
  private async checkLogRotation(): Promise<void> {
    if (!this.config.logFile || !this.config.maxSizeMB) return;
  ...
  logOperation(
    ...
    entry.performance = { duration, ...entry.performance };
  ...
  logSecurity(
    ...
    entry.security = { path, blocked, reason };
  ...
  logPerformance(
    ...
    entry.performance = { duration, fileSize, quotaUsed };
...
// Performance timing utility (unchanged)
