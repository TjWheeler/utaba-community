...
export class ApprovalQueue extends EventEmitter {
   ...
   * Approve a request
   */
  async approveRequest(requestId: string, decidedBy: string = 'user'): Promise<void> {
   ...
   * Reject a request
   */
  async rejectRequest(requestId: string, decidedBy: string = 'user'): Promise<void> {
   ...
   */
  async getStats(): Promise<ApprovalQueueStats> {
    try {
  ...
  private calculateRisk(command: string, args: string[], packageName?: string): { riskScore: number; riskFactors: string[] } {
  ...
  private async processDecision(requestId: string, status: 'approved' | 'rejected', decidedBy: string): Promise<void> {
    try {
      const pendingPath = path.join(this.pendingDir, `${requestId}.json`);
  ...
  private async initializeManifest(): Promise<void> {
    try {
      ...
      const manifest: ApprovalQueueManifest = {
        version: '1.0',
        createdAt: Date.now(),
  ...
  private async updateManifest(): Promise<void> {
    try {
      ...
      let manifest: ApprovalQueueManifest;
      try {
