...
export interface AsyncJobQueueConfig {
  baseDir?: string;              // Base directory for queue storage (defaults to process.cwd())
  queueSubdir?: string;          // Subdirectory name for queue files (defaults to 'async-queue')
  processingCapacity?: number;   // Max concurrent jobs (defaults to 5)
  cleanupInterval?: number;      // Watchdog interval in ms (defaults to 5 minutes)
  jobRetention?: number;         // Job retention period in ms (defaults to 7 days)
}
...
export class AsyncJobQueue extends EventEmitter {
  ...
  private config: Required<AsyncJobQueueConfig>;
  private isInitialized = false;
  private watchdog?: NodeJS.Timeout;
  private stats: JobQueueStats;
   ...
   */
  async submitJob(request: JobSubmissionRequest): Promise<JobRecord> {
    ...
    try {
      ...
      const job: JobRecord = {
        ...
        estimatedDuration: request.estimatedDuration || estimateExecutionDuration(
          request.operationType || classifyOperation(request.command, request.args),
          request.command,
          request.args
        ),
      ...
      this.logger.error('AsyncJobQueue', 'Failed to submit job', 'submitJob', {
        error: errorMsg,
        request: sanitizeJobForLogging(request as any)
      });
   ...
   */
  async updateJob(jobId: string, update: JobUpdate): Promise<JobRecord | null> {
    if (!this.isInitialized) {
      throw new Error('Job queue not initialized');
    }
        ...
        try {
          ...
          for (const jobDir of jobDirs) {
            ...
            try {
              ...
              // Apply filters
              if (options.operationType && job.operationType !== options.operationType) continue;
              if (options.conversationId && job.conversationId !== options.conversationId) continue;
  ...
  private async moveJobToStatus(job: JobRecord, oldStatus: JobStatus, newStatus: JobStatus): Promise<void> {
  ...
  private initializeStats(): JobQueueStats {
    return {
      totalJobs: 0,
      activeJobs: 0,
      completedJobs: 0,
  ...
  private updateStatsForStatusChange(oldStatus: JobStatus, newStatus: JobStatus): void {
