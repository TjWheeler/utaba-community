...
export class ApprovalBridge extends EventEmitter {
     ...
     */
    async createApprovalRequest(job) {
        try {
            // Generate approval request ID
            const approvalRequestId = `async_${job.id}_${Date.now()}`;
            // Calculate risk score (simple heuristic)
            let riskScore = 1;
            if (job.command === 'rm' || job.command === 'del')
                riskScore = 8;
            else if (job.command === 'npm' && job.args.includes('install'))
                riskScore = 3;
            else if (job.command === 'docker')
                riskScore = 5;
     ...
     */
    async handleApprovalDecision(approvalRequestId, decision, decidedBy, reason) {
        try {
     ...
     */
    async updateAsyncJobStatus(asyncJobId, newStatus, metadata) {
        try {
            ...
            await fs.writeFile(path.join(newJobPath, 'job.json'), JSON.stringify(updatedJob, null, 2), 'utf8');
 ...
 */
export const DEFAULT_APPROVAL_BRIDGE_CONFIG = {
