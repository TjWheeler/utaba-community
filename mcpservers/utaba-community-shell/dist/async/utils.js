 ...
 */
export function classifyOperation(command, args) {
 ...
 */
export function estimateExecutionDuration(operationType, command, args) {
...
export function calculatePollingInterval(job) {
    ...
    if (status === "executing") {
        ...
        switch (operationType) {
            ...
            case 'code_generation':
                if (elapsed < 2 * 60 * 1000)
                    return 15000; // 15s for first 2 min
                if (elapsed < 5 * 60 * 1000)
                    return 30000; // 30s for next 3 min
                return 60000; // 1min after that
            case 'build_compile':
            case 'test_suite':
                if (elapsed < 5 * 60 * 1000)
                    return 60000; // 1min for first 5 min
                if (elapsed < 15 * 60 * 1000)
                    return 120000; // 2min for next 10 min
                return 300000; // 5min after that
            case 'docker_build':
            case 'deployment':
                if (elapsed < 5 * 60 * 1000)
                    return 120000; // 2min for first 5 min
                if (elapsed < 30 * 60 * 1000)
                    return 300000; // 5min for next 25 min
 ...
 */
function getMaxExecutionTime(operationType) {
    const maxTimes = {
 ...
 */
export function createProgressMessage(job) {
    const elapsed = Date.now() - job.submittedAt;
 ...
 */
export function getJobDirectoryName(job) {
 ...
 */
export function getJobFilePaths(baseDir, job) {
 ...
 */
export function calculateJobPriority(job) {
    let priority = 100; // Base priority
    // Higher priority for user-interactive operations
    if (job.operationType === 'test_suite')
        priority += 20;
    if (job.operationType === 'build_compile')
        priority += 15;
    if (job.operationType === 'package_install')
        priority += 10;
    // Lower priority for long-running operations
    if (job.operationType === 'docker_build')
        priority -= 10;
    if (job.operationType === 'deployment')
        priority -= 5;
 ...
 */
export function sanitizeJobForLogging(job) {
